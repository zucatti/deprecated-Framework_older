	...
	exec: function(o,cb)
	{
		...
		function querycommander(o) {
			...
			if (cmd.indexOf('.sort(')>-1) {
				var fcn=cmd.split('.sort(')[1].split(')')[0];
				LIMIT.push(fcn);
			};
			...
			if (ORDERBY.length>0) {
				...
				{
					if (ORDERBY[i].indexOf('+')>-1) SQL.push(ORDERBY[i].split('+')[0].split('=')[0]);
					if (ORDERBY[i].indexOf('-')>-1) SQL.push(ORDERBY[i].split('-')[0].split('=')[0]+' DESC');
				};
			};
			// limit
			if (LIMIT.length>0) {
